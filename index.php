...
if($_SERVER["REQUEST_METHOD"] == "GET"){
  ...
  if(empty($error)){
    ...
    $user = 'u52878';
    $pass = '2527519';
    $conn = new PDO('mysql:host=localhost;dbname=u52878', $user, $pass, [PDO::ATTR_PERSISTENT => true]);
    $stmt = $conn->prepare("INSERT INTO TABLICA(name, email, year, gender, limbs, biography) VALUES (:name, :email, :year, :gender, :limbs, :biography)");
    $rez=$stmt->execute(['name'=>"$name",'email'=>"$email", 'year'=>"$year", 'gender'=>"$gender", 'limbs'=>"$kol", 'biography'=>"$biography"]);
    $id_form=$conn->lastInsertId();
    $stmt2=$conn->prepare("INSERT INTO SUSPOSOBNOSTI(immortality, passing_through_walls,levitation) VALUES (:immortality, :passing_through_walls, :levitation)");
    ...
    $stmt3=$conn->prepare("INSERT INTO TABLICA_SUPERSPOSOBNOSTI(id_DATA_FORM, id_DATA_superpower) VALUES (:id_DATA_FORM, :id_DATA_superpower)");
